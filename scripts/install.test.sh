...
if [[ "$(basename "$0")" = "curl" ]]; then
  ...
  case "$url" in
    https://github.com/logbookfordevs/logbook-waypoint/releases/latest)
      ...
      printf 'https://github.com/logbookfordevs/logbook-waypoint/releases/tag/v0.1.2'
      ;;
    https://github.com/logbookfordevs/logbook-waypoint/releases/download/v0.1.2/waypoint-cli.tar.gz)
      cp "$FAKE_ARCHIVE" "$output_path"
      ;;
    https://github.com/logbookfordevs/logbook-waypoint/releases/download/v0.1.2/waypoint-cli.tar.gz.sha256)
...
printf '#!/usr/bin/env node\nif (process.argv.includes("--version")) console.log("0.1.2");\n' > "$PAYLOAD_DIR/bin/cli.js"
printf 'export {};\n' > "$PAYLOAD_DIR/lib/server.js"
printf '{"name":"@logbookfordevs/waypoint","version":"0.1.2","type":"module"}\n' > "$PAYLOAD_DIR/package.json"
...
grep -q 'releases/download/v0.1.2/waypoint-cli.tar.gz' "$FAKE_CURL_LOG"
test -f "$INSTALL_ROOT/releases/v0.1.2/bin/cli.js"
test -f "$INSTALL_ROOT/releases/v0.1.2/lib/server.js"
test -x "$BIN_DIR/waypoint"
"$BIN_DIR/waypoint" --version | grep -q '^0.1.2$'
